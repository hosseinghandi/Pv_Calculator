...
# --- CHANGES ---
# 1. Removed type hints from all function signatures (not supported in Python 2)
# 2. Replaced f-strings with .format() or % formatting
# 3. Replaced list[str] / list[Brep] / list[int] return type hints (not supported)
# 4. range() already returns a list in Python 2, but list() wrapping is kept for safety
# 5. next(reader) is valid in Python 2.6+, kept as-is
...
def cal_elevation(latitude, longitude, timezone):
    try:
        if all([latitude is not None, longitude is not None, timezone is not None]):
            ...
            # Solar Declination (δ)
            delta = 23.45 * math.sin(math.radians((360.0 / 365) * (N - 81)))
            # Hour Angle (γ)
            lstm = 15 * float(timezone)
            TC1 = 4 * (longitude - lstm)
            B = math.radians((360.0 / 365) * (N - 81))
            ...
            raise ValueError("Latitude, longitude, or timezone is missing.")
    except Exception as e:
        raise RuntimeError("Solar elevation calculation failed: {0}".format(e))
...
def epw_open(epw_path):
    ...
    try:
        with open(epw_path, 'r') as epw:
            ...
            return (
                (
                    ...
                    "Latitude: {1:.2f}\n"
                    "Longitude: {2}\n"
                    "Weather Data Source: EPW file based on model years "
                    "{3}-{4}\n\n"
                ).format(
                    location_data[0],
                    float(location_data[1]),
        ...
        raise IOError("EPW file not found at path: {0}".format(epw_path))
    except Exception as e:
        raise RuntimeError("[epw_open] Something went wrong: {0}".format(e))
...
def addTiltSurface(roofs, ang):
    try:
        if roofs:
            ...
            raise ValueError("The roof input is missed")
    except Exception as e:
        raise RuntimeError("Something went wrong: {0}".format(e))
...
def roof_picker(buildings, ang):
    ...
    try:
        if len(buildings) > 0:
            ...
            raise ValueError("Building geometry could not be deconstructed!")
        ...
        raise RuntimeError("Something went wrong while handling buildings: {0}".format(e))
...
try:
    ...
    ang    = 0     if angle_ is None else angle_
    ...
    pv_data_pack = json.dumps({
        ...
        "period": "{0}".format('yearly' if not period else 'monthly')
    })
    period_hoys = [] if not period else data_maker(period)
