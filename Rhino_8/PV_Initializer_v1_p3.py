...
class MyComponent(Grasshopper.Kernel.GH_ScriptInstance):
    def RunScript(self,
            _buildings: System.Collections.Generic.List[Rhino.Geometry.GeometryBase],
            _EPW_file,
            angle_: float,
            monthly_period):
        ...
        def cal_elevation(latitude : float, longitude : float, timezone : float) -> float:
            try:
        ...
        def epw_open(epw_path : str) -> list[str]: 
            if not epw_path:
        ...
        def addTiltSurface(roofs : list[Brep] , ang : float) -> Brep :
            try:
                if (roofs):
                    if len(roofs) == 1:
        ...
        def roof_picker(buildings : list[Brep], ang : float) -> list[Brep]:
            roofs=[]
            try:
                if len(buildings) > 0:
        ...
        def data_maker(period: bool) -> list[int]:
            required_hours = [] 
            if period: 
        ...
        try:
            ...
            ghc.add_warning(f"{e}")
        return roofs, pv_data_pack, period_hoys
